...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    ...
    private void sendToWearable(double high, double low, int weatherId) {
        PutDataMapRequest requestMap = PutDataMapRequest.create("/weather_data");

        requestMap.getDataMap().putString("max_temp", Utility.formatTemperature(getContext(), high));
        requestMap.getDataMap().putString("min_temp", Utility.formatTemperature(getContext(), low));
        requestMap.getDataMap().putInt("weather_image_id", weatherId);
    ...
    public void onConnected(Bundle bundle) {
        Log.i(LOG_TAG, "Wearable connected");
        ...
        Cursor cursor = new CursorLoader(getContext(), weatherLocation, DetailFragment.DETAIL_COLUMNS, null, null, sortOrder).loadInBackground();
        if (cursor != null && cursor.getCount() > 0) {
            ...
            int weatherId = cursor.getInt(DetailFragment.COL_WEATHER_CONDITION_ID);
            sendToWearable(high, low, weatherId);
        }
    }
